...
class Providers extends CI_Controller {
    ...
    {   
        ...
        {   
            //$ContractDate = explode(" - ",$this->input->post('ContractDate'));
			...
			$params = array(
				...
				'OwnersPostalCode' => $this->input->post('PostCode'),
				...
				'AvailableVehicle' => $this->input->post('TotalVehicle'),
    ...
    {   
        ...
        if(isset($provider['ID']))
        {
            ...
            {   
				...
				exit;*/
    			$data['cities'] = $this->db->get('city')->result_array();
				$data['CloseDates'] = $this->ci_model->get_close_dates($ID);
    ...
    {   
        ...
        if(isset($provider['ID']))
        {
        	...
        	{                  	
				...
				{
					/*$OpenHour = explode(" ",$this->input->post('OpenHour'));
					$OpenHour = $OpenHour[0].$OpenHour[1].$OpenHour[2]." ".$OpenHour[3];
					
					$CloseHour = explode(" ",$this->input->post('CloseHour'));
					$CloseHour = $CloseHour[0].$CloseHour[1].$CloseHour[2]." ".$CloseHour[3];*/
					...
					$CloseDates = array_filter($this->input->post('CloseDates'));
					$this->ci_model->update_holiday('holiday_list',$CloseDates,$ID);  
        ...
        if(isset($provider['ID']))
        {
        	...
        	{                  	
				...
				$params = array(
					...
					'PostCode' => $this->input->post('OwnersPostalCode'),
